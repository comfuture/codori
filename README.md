...
The dashboard landing screen also offers **Start voice companion**. It creates a persistent projectless `gpt-5.6-luna` thread with `xhigh` reasoning, stays on `/`, and replaces the landing content with a centered server avatar rather than opening the chat transcript. The area above that avatar shows subtitles for what Codori heard and answered, using the same two-exchange window and inactivity timeout as the in-thread companion bubble. Both subtitle surfaces are translucent with a backdrop blur, so the content behind them stays faintly visible. Select the centered avatar to stop the session; navigating elsewhere keeps the same session alive in the normal corner companion.
