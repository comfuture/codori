...
- With a tracked left hand and no active left controller, hold the raised back-of-hand pose for `700 ms`. Lowering it beyond the close threshold for `350 ms` dismisses the window. Wrist rotation and temporary tracking loss do not dismiss an open window, so the right hand can approach and select while the left hand is partially occluded.
...
- Placement uses the left grip or WebXR `wrist` direction at a fixed `0.4 m` viewer distance. The physical surface is `0.24 m × 0.32 m`, exactly one third of the previous window and text scale, and keeps following allowed lateral/vertical wrist movement. If the wrist is temporarily lost, the window stays at the same close viewer-relative fallback instead of jumping away or closing. WebXR exposes no elbow joint, so this remains a wrist/grip-and-view approximation rather than full forearm tracking.
