...
- When no exposed mapped menu component or currently tracked hand pose can invoke status, an in-canvas bottom-right `Menu` is shown. This includes controllers whose app/system button is reserved or unmapped; their target ray can select the fallback. A DOM-overlay button mirrors it when the optional DOM Overlays feature is granted. Both disappear when a mapped menu control or tracked hand pose becomes usable; screen/gaze can open the menu but cannot bypass `controller-or-touch` action policy.
